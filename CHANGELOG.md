...
- [Reference documentation](https://inspect.ai-security-institute.uk/reference/) for Python API and CLI commands.
- Add support for [clustered standard errors](https://inspect.ai-security-institute.uk/scorers.html#clustered-standard-errors) via a new `cluster` parameter for the `stderr()` metric.
- Improvements to [scoring workflow](https://inspect.ai-security-institute.uk/scorers.html#sec-scorer-workflow) (`inspect score` command and `score()` function).
...
- Add [OpenRouter](https://inspect.ai-security-institute.uk/providers.html#openrouter) model provider.
...
- [Agent Bridge](https://inspect.ai-security-institute.uk/agent-bridge.html) for integrating external agent frameworks with Inspect.
- [Goodfire](https://inspect.ai-security-institute.uk/models.html#goodfire) model provider.
...
- Beta version of [computer()](https://inspect.ai-security-institute.uk/tools.html#sec-computer) tool which models with a computer desktop environment.
...
- Limits: Enforce [custom limits](https://inspect.ai-security-institute.uk/errors-and-limits.html#custom-limit) for samples by raising `SampleLimitExceededError`.
- Tasks: Optional ability for solvers to [yield scores](https://inspect.ai-security-institute.uk/solvers.html#sec-scoring-in-solvers) for a task.
...
- Support for [audio and video](https://inspect.ai-security-institute.uk/multimodal.html) inputs for Open AI and Google Gemini models.
...
- [Tracing API](https://inspect.ai-security-institute.uk/tracing.html#tracing-api) for custom trace logging.
...
- [Human Agent](https://inspect.ai-security-institute.uk/human-agent.html) solver for human baselining of computing tasks.
- [Typed interfaces](https://inspect.ai-security-institute.uk/typing.html) to `Sample` store and metadata using Pydantic models.
- [Approval policies](https://inspect.ai-security-institute.uk/approval.html#task-approvers) can now be defined at the `Task` level (`eval` level approval policies take precedence).
...
- [Tracing](https://inspect.ai-security-institute.uk/tracing.html) for diagnosing runs with unterminated action (e.g. model calls, docker commands, etc.).
...
- Scoring: When using a dictionary to map metrics to score value dictionaries, you may now use globs as keys. See our [scorer documentation](https://inspect.ai-security-institute.uk/scorers.html#sec-multiple-scorers) for more information.
...
- Use [tool views](https://inspect.ai-security-institute.uk/approval.html#tool-views) when rendering tool calls in Inspect View.
...
- [eval](https://inspect.ai-security-institute.uk/eval-logs.html#sec-log-format) is now the default log format (use `--log-format=json` to use old format).
...
- [time_limit](https://inspect.ai-security-institute.uk/errors_and_limits.html#sample-limits) option for specifying a maximum execution time for samples.
- [read_eval_log_samples()](https://inspect.ai-security-institute.uk/eval-logs.html#streaming) function for streaming reads of `.eval` log files.
...
- New binary [log format](https://inspect.ai-security-institute.uk/eval-logs.html#sec-log-format) which yields substantial size and speed improvements (JSON format log files are still fully supported and utilities for converting between the formats are provided).
...
- [ToolDef](https://inspect.ai-security-institute.uk/tools.html#sec-dynamic-tools) class for dynamically creating tool definitions.
...
- [Approval mode](https://inspect.ai-security-institute.uk/approval.html) for extensible approvals of tool calls (human and auto-approvers built in,  arbitrary other approval schemes via extensions).
- [Trace mode](https://inspect.ai-security-institute.uk/interactivity.html#sec-trace-mode) for printing model interactions to the terminal.
- Add `as_dict()` utility method to `Score`
- [Sample limits](https://inspect.ai-security-institute.uk/errors_and_limits.html#sample-limits) (`token_limit` and `message_limit`) for capping the number of tokens or messages used per sample ( `message_limit` replaces deprecated `max_messages`).
...
- [Web Browser](https://inspect.ai-security-institute.uk/tools.html#sec-web-browser) tool which provides a headless Chromium browser that supports navigation, history, and mouse/keyboard interactions.
...
- Added [fork()](https://inspect.ai-security-institute.uk/agents-api.html#sec-forking) function to fork a `TaskState` and evaluate it against multiple solvers in parallel.
...
- [basic_agent()](https://inspect.ai-security-institute.uk/agents.html#sec-basic-agent) that provides a ReAct tool loop with support for retries and encouraging the model to continue if its gives up or gets stuck.
- [score()](https://inspect.ai-security-institute.uk/solvers.html#sec-scoring-in-solvers) function for accessing scoring logic from within solvers.
- Ability to [publish](https://inspect.ai-security-institute.uk/log-viewer.html#sec-publishing) a static standalone Inspect View website for a log directory.
...
- [Eval Sets](https://inspect.ai-security-institute.uk/eval-sets.html) for running groups of tasks with automatic retries.
- [Per-sample](https://inspect.ai-security-institute.uk/sandboxing.html#sec-per-sample-sandbox) Sandbox environments can now be specified (e.g. allowing for a distinct Dockerfile or Docker compose file for each sample).
- [input_screen()](https://inspect.ai-security-institute.uk/interactivity.html) context manager to temporarily clear task display for user input.
...
- [Store](https://inspect.ai-security-institute.uk/agents-api.html#sharing-state) for manipulating arbitrary sample state from within solvers and tools.
- [Transcript](https://inspect.ai-security-institute.uk/agents-api.html#transcripts) for detailed sample level tracking of model and tool calls, state changes, logging, etc.
- [Subtasks](https://inspect.ai-security-institute.uk/agents-api.html#sec-subtasks) for delegating work to helper models, sub-agents, etc.
- Integration with Anthropic [prompt caching](https://inspect.ai-security-institute.uk/caching.html#sec-provider-caching).
- [fail_on_error](https://inspect.ai-security-institute.uk/errors-and-limits.html#failure-threshold) option to tolerate some threshold of sample failures without failing the evaluation.
...
- [vLLM](https://inspect.ai-security-institute.uk/models.html#sec-vllm) model provider.
- [Groq](https://groq.com/) model provider.
- [Google Vertex](https://inspect.ai-security-institute.uk/models.html#google-vertex) model provider.
- [Reduce scores](https://inspect.ai-security-institute.uk/scorers.html##sec-reducing-epoch) in multi-epoch tasks before computing metrics (defaults to averaging sample values).
- Replace the use of the `bootstrap_std` metric with `stderr` for built in scorers (see [rationale](https://inspect.ai-security-institute.uk/scorers.html#stderr-note) for details).
- Option to write Python logger entries to an [external file](https://inspect.ai-security-institute.uk/log-viewer.html#sec-external-file).
...
- [Multiple Scorers](https://inspect.ai-security-institute.uk/scorers.html#sec-multiple-scorers) are now supported for evaluation tasks.
- [Multiple Models](https://inspect.ai-security-institute.uk/parallelism.html#sec-multiple-models) can now be evaluated in parallel by passing a list of models to `eval()`.
...
-   [Sandbox Environments](https://inspect.ai-security-institute.uk/sandboxing.html) for executing tool code in a sandbox.
-   [Caching](https://inspect.ai-security-institute.uk/caching.html) to reduce the number of model API calls made.
